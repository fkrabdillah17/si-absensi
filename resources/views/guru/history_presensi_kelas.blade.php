...
@section('title')
    History Presensi | {{ $mapel }} | Daftar Kelas
@endsection

@section('content')
    <div class="mb-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        @foreach ($kelas as $item)
            <a href="{{ route('history.presensi.pertemuan', [$item->id_mapel, $item->id_kelas]) }}">
                <div class="shadow-xs flex items-center rounded-lg bg-white p-4">
                    ...
                    <div>
                        <p class="mb-2 text-sm font-medium text-gray-600">
                            {{ $item->getKelas->kelas . ' ' . $item->getKelas->kode_kelas . ' ' . $item->getKelas->getJurusan->kode_jurusan }}
                        </p>
                    </div>
                </div>
            </a>
