    ...
    <div class="mb-6 flex w-full min-w-0 flex-col break-words rounded-xl bg-white shadow-lg">
        <div class="mb-0 rounded-t border-0 px-4 py-3">
            <div class="flex flex-row items-center">
                <a type="button" href="{{ route('jadwal.create') }}" class="btn-primary btn-sm btn">Tambah</a>
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($data as $jadwal)
                        <tr>
                            ...
                            <td class="uppercase">{{ $jadwal->getMapel->mapel }}</td>
                            <th class="uppercase">
                                {{ $jadwal->getKelas->kelas . ' ' . $jadwal->getKelas->kode_kelas . ' ' . $jadwal->getKelas->getJurusan->jurusan }}</th>
                            <th>{{ $jadwal->getGuru->nama }}</th>
                            </th>
                            <th class="flex flex-row justify-around">
                                <a class="btn-ghost btn" href="{{ route('jadwal.edit', $jadwal->id) }}">
                                ...
                                <label for="my-modal-{{ $jadwal->id }}" class="btn-ghost btn">
    ...
    @foreach ($data as $modal)
        ...
        <div class="modal">
            <div class="modal-box relative">
                ...
                <h3 class="text-center text-lg font-bold">Apakah anda yakin akan menghapus data?</h3>
                <div class="my-4 flex flex-row justify-around">
                    <label for="my-modal-{{ $modal->id }}" class="btn-warning btn capitalize">Tidak</label>
                    <form action="{{ route('jadwal.destroy', $modal->id) }}" method="post">
                        @csrf
                        @method('delete')
